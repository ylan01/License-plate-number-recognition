...
void LPR::load(String path)
{
	...
	cvtColor(srcImage, closeImage1, CV_8UC1);
	
	for (int i = 0; i < template_len; i++)
	{
		stringstream stream;
		stream << "D:\\TuXiang\\车牌号识别\\hjh1\\hjh1\\pictures\\num_";
		stream << i;
		stream << ".bmp";
...
void LPR::getMaxArea()
{
	...
	//cv::findContours(closeImage,
	//	contours, // a vector of contours
	//	CV_RETR_EXTERNAL, // 提取外部轮廓
	//	CV_CHAIN_APPROX_NONE); // all pixels of each contours
	vector<vector<Point>> contours;
	vector<Vec4i> hierarchy;
	findContours(closeImage, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
...
void LPR::recognition()
{
	...
	cout << endl;
	/*imshow("charMat[0]", charMat[0]);
	imshow("charMat[1]", charMat[1]);
	imshow("charMat[2]", charMat[2]);
	imshow("charMat[3]", charMat[3]);
	imshow("charMat[4]", charMat[4]);
	imshow("charMat[5]", charMat[5]);
	imshow("charMat[6]", charMat[6]);*/
}
